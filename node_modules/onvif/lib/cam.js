...
Cam.prototype._envelopeHeader = function (openHeader) {
	...
	var header = '<s:Envelope xmlns:s="http://www.w3.org/2003/05/soap-envelope" xmlns:a="http://www.w3.org/2005/08/addressing">' +
		'<s:Header>' +
		'<a:MessageID>' + (new Date(Date.now() + (this.timeShift || 0))).toISOString() + '</a:MessageID>' +
		'<a:ReplyTo><a:Address>http://' + this.localAddress + '</a:Address></a:ReplyTo>';
	...
	if (!(openHeader !== undefined && openHeader)) {
		header += this._envelopeHeaderClose();
	}
	return header;
};

Cam.prototype._envelopeHeaderClose = function () {
	return '</s:Header>' +
	'<s:Body xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema">';
};
