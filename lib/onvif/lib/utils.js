...
const parseSOAPString = function(xml, callback) {
	Homey.app.updateLog("parseSOAPString: " + Homey.app.varToString(xml) + "\n")
	...
	try {
		xml2js.parseString(
			...
			, function(err, result) {
				if (!result || !result['envelope'] || !result['envelope']['body']) {
					Homey.app.updateLog("!!!!! parseSOAPString error 1: " + Homey.app.varToString(xml) + "\n", true);
					...
					if (!err && result['envelope']['body'][0]['fault']) {
						...
						Homey.app.updateLog("!!!!! parseSOAPString error 2: " + Homey.app.varToString(err) + "   --- xml: " + Homey.app.varToString(xml) + "\n", true);
		...
		Homey.app.updateLog("!!!!! parseSOAPString error 3: " + Homey.app.varToString(err) + "   --- xml: " + Homey.app.varToString(xml) + "\n", true);
		callback(err, '', xml);
	}
};
