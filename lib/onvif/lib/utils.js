		...
		xml2js.parseString(
			...
			function(err, result) {
				if (!result || !result.envelope || !result.envelope.body) {
					return callback(new Error('Wrong ONVIF SOAP response'), null, xml);
				} else {
					if (!err && result.envelope.body[0] && result.envelope.body[0].fault) {
						...
						err = new Error('ONVIF SOAP Fault');
					}
					return callback(err, result.envelope.body, xml);
				}
			}
		);
	} catch (err) {
