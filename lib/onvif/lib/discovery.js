...
Discovery.probe = function(options, callback) {
	...
	const listener = function(msg, rinfo) {
		parseSOAPString(msg.toString(), function(err, data, xml) {
			...
			if (err || !data[0].probeMatches) {
				 ...
				 * @type {string}
				 */
				//console.log( "Wrong SOAP message: ", xml);
				...
				if (!cams[camAddr]) {
					...
					if (options.resolve !== false) {
						...
						let camUris = data.probeMatches.probeMatch.XAddrs.split(' ').map(url.parse),
							camUri = matchXAddr(camUris, rinfo.address)
						;
						cam = {
							hostname: camUri.hostname,
	...
	setTimeout(function() {
		...
		socket.close();
		callback.call(this, errors.length ? errors : null, Object.keys(cams).map(function(addr) { return cams[addr]; }));
