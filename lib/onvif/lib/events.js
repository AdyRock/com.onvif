...
module.exports = function (Cam) {
	...
	Cam.prototype.getEventProperties = function (callback) {
		this._request({
			...
			if (!err) {
				...
				this.homey.app.updateLog("!!!!! getEventServiceCapabilities error: " + err.message + "\n", 3);
			}
	...
	Cam.prototype.createPullPointSubscription = function (callback) {
		...
		this._request({
			...
			try {
				...
				this.homey.app.updateLog("!!!!! createPullPointSubscription callback error: " + err.message + "\n", 0);
	...
	Cam.prototype._eventPull = function (err) {
		if (err) {
			if ((this.listeners('event').length === 0) || !this.events.subscription) {
				delete this.events.terminationTime;
		...
		else if (this.listeners('event').length && this.events.subscription) { // check for event listeners, if zero, stop pulling
			this.pullMessages({
