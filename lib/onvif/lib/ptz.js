...
module.exports = function(Cam) {
    ...
    function getProfileToken(cam, options) {
        if (options && options.profileToken) {
    ...
    Cam.prototype.getPresets = function(options, callback) {
        ...
        const profileToken = getProfileToken(this, options);
        var requestGetPresets = function(ptzNs, cb) {
            this._request({
                service: 'ptz'
                , body: this._envelopeHeader() +
                '<GetPresets xmlns="' + ptzNs + '">' +
        ...
        requestGetPresets('http://www.onvif.org/ver20/ptz/wsdl', function(err, data, xml) {
            const errMessage = err ? (err.message || String(err)) : '';
            if (err && /item not found|soap fault/i.test(errMessage)) {
                return requestGetPresets('http://www.onvif.org/ver10/ptz/wsdl', function(fallbackErr, fallbackData, fallbackXml) {
                    if (!fallbackErr) {
                        this.presets = {};
                        var fallbackPresets = linerase(fallbackData).getPresetsResponse.preset;
                        if (typeof fallbackPresets !== 'undefined') {
                            if (!Array.isArray(fallbackPresets)) {
                                fallbackPresets = [fallbackPresets];
                            }
                            fallbackPresets.forEach(function(preset) {
                                this.presets[preset.name] = preset.$.token;
                            }.bind(this));
                        }
                    }
                    if (callback) {
                        callback.call(this, fallbackErr, this.presets, fallbackXml);
                    }
                }.bind(this));
            }
