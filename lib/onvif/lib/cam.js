...
Cam.prototype._request = function (options, callback) {
	...
	try {
		var req = http.request(reqOptions, function (res) {
			...
			res.on('end', function () {
				...
				Homey.app.updateLog("\n_request response (" + this.hostname + "): " + Homey.app.varToString(xml) + "\n", 2);
		...
		Homey.app.updateLog("Request error (" + this.hostname + "): " + Homey.app.varToString(err), 0);
	...
	req.setTimeout(this.timeout, function () {
		...
		Homey.app.updateLog("Request timeout (" + this.hostname + ")", 0);
...
Cam.prototype.getServices = function (includeCapability, callback) {
	...
	this._request({
		...
		if (!err) {
			 ...
			 * @type {Cam~Services}
			 */
			Homey.app.updateLog("getServices (" + this.hostname + "): " + Homey.app.varToString(data), 2 );
