...
Cam.prototype.connect = function (callback) {
	...
	this.getSystemDateAndTime(function (err, date, xml) {
		if (err) {
			Homey.app.updateLog("getSystemDateAndTime error:" + Homey.app.varToString(err), 0);
			return callback.call(this, err, null, xml);
		}
		this.getServices(true, function (err) {
			if (err) {
				Homey.app.updateLog("getServices error:" + Homey.app.varToString(err), 0);
				return this.getCapabilities(function (err, data, xml) {
					if (err) {
						Homey.app.updateLog("getCapabilities error:" + Homey.app.varToString(err), 0);
						return callback.call(this, err, null, xml);
					}
		...
		function callUpstartFunctions() {
			Homey.app.updateLog("callUpstartFunctions: Started *****");
					...
					fun.call(this, function (err) {
						if (err) {
							...
							if (!--count) {
								 ...
								 */
								this.emit('connect');
								Homey.app.updateLog("callUpstartFunctions: Finished -----");
								if (callback) {
...
Cam.prototype._request = function (options, callback) {
	...
	// console.log(" ");
	// console.log("-------------------------------------------------------------");
	// console.log("request: ", options.service);
	...
	Homey.app.updateLog("\n_request: " + Homey.app.varToString(reqOptions) + "\nBody: " + Homey.app.varToString(options.body) + "\n", 2);
	...
	try {
		var req = http.request(reqOptions, function (res) {
			...
			res.on('end', function () {
				...
				Homey.app.updateLog("\n_request response: " + Homey.app.varToString(xml) + "\n", 2);
		...
		Homey.app.updateLog("Request error: " + Homey.app.varToString(err), 0);
	...
	req.setTimeout(this.timeout, function () {
		...
		Homey.app.updateLog("Request timeout", 0);
...
Cam.prototype.getSystemDateAndTime = function (callback) {
	...
	Homey.app.updateLog("getSystemDateAndTime: Started *****");
	this._request({
		...
		if (!err) {
			if (data && data[0] &&
				...
				try {
					...
					Homey.app.updateLog("getSystemDateAndTime: Finished -----");
					callback.call(this, err, time, xml);
				} catch (err) {
					Homey.app.updateLog("getSystemDateAndTime: !err Error " + err + "\nxml:\n" + xml + "\ndata:\n" + data ? "\ndata:\n" + Homey.app.varToString(data) : "", 0);
				...
				if (err) {
					...
					if (data && data[0] &&
						...
						try {
							...
							Homey.app.updateLog("getSystemDateAndTime: err err Error " + err + "\nxml:\n" + xml + "\ndata:\n" + data ? "\ndata:\n" + Homey.app.varToString(data) : "", 0);
							callback.call(this, err, null, xml);
						}
					}
					else{
						var time =  Date.now();
						Homey.app.updateLog("getSystemDateAndTime: Finished using Homey time-----");
						callback.call(this, err, time, xml);
				}
				}
			}.bind(this));
...
Cam.prototype.getCapabilities = function (callback) {
	Homey.app.updateLog("getCapabilities: Started *****");
	...
	this._request({
		...
		if (!err) {
			...
			Homey.app.updateLog("getCapabilities: Finished -----");
...
Cam.prototype.getActiveSources = function () {
	 ...
	 */
	this.activeSources = [];
	Homey.app.updateLog("VideoSources: " + Homey.app.varToString(this.videoSources));
	...
	this.videoSources.forEach(function (videoSource, idx) {
		...
		Homey.app.updateLog("ActiveSource [" + idx + "] = " + Homey.app.varToString(this.defaultProfiles[idx].videoEncoderConfiguration));
		Homey.app.updateLog("VideoSource [" + idx + "] = " + Homey.app.varToString(videoSource));
...
Cam.prototype.getServices = function (includeCapability, callback) {
	Homey.app.updateLog("getServices: Started *****");
	...
	this._request({
		...
		if (!err) {
			 ...
			 * @type {Cam~Services}
			 */
			Homey.app.updateLog("getServices: " + Homey.app.updateLog(data), 2 );
			...
			this.services.forEach((service) => {
				if (service.namespace){
					var searchValue = '.org/';
					if (service.namespace.search(searchValue) > 0){
						let namespaceSplitted = service.namespace.split(searchValue)[1].split('/');
					...
					Homey.app.updateLog("getServices: Missing namespace for service " + service);
				}
			});
			Homey.app.updateLog("getServices: Finished -----");
