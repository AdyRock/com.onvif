...
Cam.prototype._request = function(options, callback){
    ...
    if (options.body === undefined) {
        return callback(new Error('\'options.body\' must be a defined'));
    }
...
Cam.prototype._requestPart2 = function(options, callback) {
    ...
    else
    {
        reqOptions = {
            ...
            port: this.port,
            path: options.service ? (this.uri[options.service] ? this.uri[options.service].path : options.service) : this.path
        };
    }
    ...
    let replyTimeout = options.replyTimeout ? options.replyTimeout : this.timeout;
    if (replyTimeout < reqOptions.timeout + 15000)
    {
        replyTimeout = reqOptions.timeout + 15000;
    }
    req.setTimeout(replyTimeout, function() {
        _this.homey.app.updateLog('Request setTimeout (' + _this.hostname + ')', 1);
        req.abort();
    });

    req.on('error', function(err) {
        _this.homey.app.updateLog('Request error (' + _this.hostname + '): ' + err.message, 1);
        if (callbackExecuted === true) {
            return;
        }
        callbackExecuted = true;
...
Cam.prototype.getActiveSources = function() {
    ...
    this.videoSources.forEach(function(videoSource, idx) {
		...
		{
			this.activeSources[idx].encoding = videoSource.encoding ? videoSource.encoding : '';
			this.activeSources[idx].width = videoSource.resolution ? videoSource.resolution.width : '';
			this.activeSources[idx].height = videoSource.resolution ? videoSource.resolution.height : '';
			this.activeSources[idx].fps = videoSource.framerate ? videoSource.framerate : '';
			this.activeSources[idx].bitrate = videoSource.rateControl ? videoSource.rateControl.bitrateLimit : '';
		}
