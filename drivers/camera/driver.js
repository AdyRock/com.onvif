  ...
  async onPair(session) {
    ...
    session.setHandler('list_devices', async () => {
      ...
      if (tempCam) {
        this.homey.app.updateLog(
          `list_devices2: Multiple Sources = ${this.homey.app.varToString(
            tempCam.videoSources,
            null,
            3
          )}`,
          1
        ...
        for (let i = 0; i < tempCam.videoSources.length; i++) {
          ...
          this.homey.app.updateLog(
            `list_devices2: Adding source ${this.lastURN}${channelSuf} to list`,
            1
          );
          const data = {
        ...
        this.homey.app.updateLog(
          `list_devices2: Listing ${this.homey.app.varToString(devices, null, 3)}`,
          1
        );
        return devices;
      }
    ...
    session.setHandler('manual_connection', async data => {
      ...
      try {
        ...
        this.homey.app.updateLog(`Failed to connect to camera, error: ${err.message}, ${err}`, 0);
        throw new Error(`Discovery error: ${err.message}`, { cause: err });
      }

      this.homey.app.updateLog(
        `Credentials OK. Adding ${this.homey.app.varToString(cam.videoSources)}`,
        1
      );
      ...
      if (Array.isArray(cam.videoSources) && cam.videoSources.length > 1) {
        ...
        this.homey.app.updateLog(
          `Multiple source found. Adding ${cam.videoSources.length} more devices`,
          1
        );
        tempCam = cam;
        listDevices = 2;
